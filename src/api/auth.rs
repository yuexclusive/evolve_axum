...
use crate::{env::ENV, error::AppError, error::ErrorResponse};
use crate::{error::AuthError, model::user::CurrentUser};
use crate::{response, service::user as user_service};
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 401, description = "Wrong credentials", body = ErrorResponse),
        (status = 500, description = "Token creation error", body = ErrorResponse),
    )
)]
...
#[utoipa::path(
    ...
    path = "/user_info",
    responses(
        (status = 200, description = "successfully", body = DataResponse<CurrentUser>),
...
pub async fn user_info(claims: Claims) -> Result<Json<DataResponse<CurrentUser>>, AppError> {
    match claims.ty {
        AuthorizeType::User => {
            let current_user = user_service::get_current_user(&claims.sub).await?;
            Ok(Json(response::data(current_user)))
        }
        _ => AppError::Validate {
...
#[openapi(
    ...
    components(schemas(AuthReq, AuthResp, ErrorResponse, CurrentUser))
)]
pub struct JWTApi;
